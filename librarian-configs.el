...
(defun librarian-configs--build-modules-cache ()
  (let* ((root librarian-configs--modules-dir)
         (groups (f-directories librarian-configs--modules-dir))
...
(defun librarian-configs--edit-bindings ()
  (interactive)
  (unless librarian-configs--modules-cache (librarian-configs--build-modules-cache))
  (let* ((bindings (-select #'(lambda (x) (f-exists? (f-join librarian-configs--modules-dir  x librarian-configs--binding-file-name))) librarian-configs--modules-cache))
         (chosen (ivy-read "Select Module Bindings: " librarian-configs--modules-cache :require-match t))
         (binding (f-join librarian-configs--modules-dir chosen librarian-configs--binding-file-name))
        )
    (if (f-exists? binding)
      (find-file binding)
...
(defun librarian-configs--edit-vars ()
  (interactive)
  (unless librarian-configs--modules-cache (librarian-configs--build-modules-cache))
  (let* ((vars (-select #'(lambda (x) (f-exists? (f-join librarian-configs--modules-dir  x librarian-configs--var-file-name))) librarian-configs--modules-cache))
         (chosen (ivy-read "Select Module Vars: "  librarian-configs--modules-cache :require-match t))
         (varf (f-join librarian-configs--modules-dir  chosen librarian-configs--var-file-name))
        )
    (if (f-exists? varf)
      (find-file varf)
...
(defun librarian-configs--edit-config ()
  (interactive)
  (unless librarian-configs--modules-cache (librarian-configs--build-modules-cache))
  (let* ((config (-select #'(lambda (x) (f-exists? (f-join librarian-configs--modules-dir  x librarian-configs--config-file-name))) librarian-configs--modules-cache))
         (chosen (ivy-read "Select Module Config: " librarian-configs--modules-cache :require-match t))
         (binding (f-join librarian-configs--modules-dir  chosen librarian-configs--config-file-name))
        )
    (if (f-exists? binding)
      (find-file binding)
      (message "Doesnt Exist: %s" binding)
...
(defun librarian-configs--edit-spec-defs ()
  ...
  (let* ((config (-select #'(lambda (x) (f-exists? (f-join librarian-configs--modules-dir  x librarian-configs--config-file-name))) librarian-configs--modules-cache))
         (chosen (ivy-read "Select Module Config: " librarian-configs--modules-cache :require-match t))
         (binding (f-join librarian-configs--modules-dir  chosen librarian-configs--spec-def-file-name))
        )
    (if (f-exists? binding)
      (find-file binding)
      (message "Doesnt Exist: %s" binding)
